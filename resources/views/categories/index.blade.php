            ...
            @forelse ($categories as $category)
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="py-2 px-4 border-b">
                        <form onsubmit="return confirm('Apakah Anda Yakin ?');" action="{{ route('categories.destroy', $category->id) }}" method="POST" class="flex items-center space-x-2">
                            <a href="{{ route('categories.edit', $category->id) }}" class="bg-yellow-500 text-white px-3 py-1 rounded-md text-sm hover:bg-yellow-600">EDIT</a>
                            @csrf
                            @method('DELETE')
